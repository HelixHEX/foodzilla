...
router.post('/toggle-add-options', async (req: express.Request, res: express.Response) => {
    const {body} = req;
    const {sessionId} = body
    try {
        ...
        if (session) {
            if (session.createdBy === req.user.userId) {
                await prisma.vote_Session.update({where: {id: sessionId}, data: {add_options: !session.add_options}})
                res.json({success: true}).status(200)
            } else {
